...
function bagla({ soket, io, oyun, ogretmenMi, tasarimlar, galeri, girisYayinla, panelYayinla, galeriYayinla, herkeseDurum, icerik }) {
  ...
  // Soru gezinme — panelde YALNIZ senkron modlarda görünür (bireysel modda gizli).
  // Atlanan soru, cevaplamamış öğrenciler için "atlandi" olarak kaydedilir.
  ogretmenOlayi('ogretmen:oncekiSoru', () => oyun.oncekiSoru());
  ogretmenOlayi('ogretmen:soruyaGit', (veri) => oyun.soruyaGit(veri && veri.sira));
  ...
  ogretmenOlayi('ogretmen:tasarimGonder', (veri) => {
    ...
    const sonuc = oyun.ozelSoru(icerik.tasarimdanSoru(tasarim));
  ...
  ogretmenOlayi('ogretmen:oturumAc', (veri) => {
    const sonuc = oyun.oturumAc(veri && veri.grup, veri && veri.etiket);
  ...
  // ⏹ Etkinliği Bitir — grup/etkinlik değiştirmenin TEK yolu.
  // Oturum BOŞTA'ya döner, cihazlardaki kayıtlı isimler temizlenir.
  ogretmenOlayi('ogretmen:etkinligiBitir', () => {
    const sonuc = oyun.etkinligiBitir();
    for (const id of sonuc.dusenSoketler || []) {
      io.to(id).emit('cikarildin', { sebep: 'etkinlikBitti' });
    }
    return { tamam: true };
  });
  ...
  // Öğrenciyi sahneden çıkar: oyuncu silinir, cihazındaki isim temizlenir
  ogretmenOlayi('ogretmen:sahnedenCikar', (veri) => {
    const sonuc = oyun.sahnedenCikar(veri && veri.kod);
    if (sonuc.tamam && sonuc.eskiSoket) {
      io.to(sonuc.eskiSoket).emit('cikarildin', { sebep: 'cikarildi' });
    }
    return sonuc.hata ? { hata: sonuc.hata } : { tamam: true, isim: sonuc.isim };
  });
  ...
  ogretmenOlayi('ogretmen:listeGuncelle', (veri) => {
    const sonuc = liste.guncelle(veri && veri.kod, (veri && veri.degisiklik) || {});
    if (!sonuc.tamam) return sonuc;

    // Pasifleştirilen ya da başka gruba taşınan öğrenci sahnedeyse oturum
    // listesinden de iner; cihazı isim seçme ekranına döner.
    const o = sonuc.ogrenci;
    const sahnede = oyun.oyuncular.has(o.kod);
    if (sahnede && (!o.aktif || o.grup !== oyun.oturum.grup)) {
      const cikis = oyun.sahnedenCikar(o.kod, { beklemeli: false });
      if (cikis.eskiSoket) io.to(cikis.eskiSoket).emit('cikarildin', { sebep: 'pasif' });
    }
    return sonuc;
  });
  ...
  ogretmenOlayi('ogretmen:misafirCikar', (veri) => {
    ...
    const oyuncu = oyun.oyuncular.get(kod);
    if (oyuncu) {
      // Oyundaysa cihazını da temizle; yoksa çocuk eski oyun ekranında takılı kalır
      if (oyuncu.socketId) io.to(oyuncu.socketId).emit('cikarildin', { sebep: 'cikarildi' });
      oyun.oyuncular.delete(kod);
      oyun.cevaplar.delete(kod);
    }
