...
function rotalariKur(app, { oyun, yetkiliMi }) {
  ...
  // Karne çıktı kipi: ?ad=kodlu → belgede isim geçmez (arşiv/araştırma),
  // varsayılan isimli → veli karnesi.
  const kodluMu = (istek) => istek.query.ad === 'kodlu';
  ...
  rota.get('/karne', (istek, yanit) => {
    ...
    const kodlu = kodluMu(istek);
    console.log(`[veri] karne açıldı — ${kodlu ? oyuncu.kod : oyuncu.isim + ' (' + oyuncu.kod + ')'} · ${kodlu ? 'kodlu' : 'isimli'}`);
    yanit.send(karne.tekKarne(oyun.olcme.rapor(oyuncu), oyun.ayar, kodlu));
  });
  ...
  rota.get('/karneler', (istek, yanit) => {
    ...
    const raporlar = [...oyun.oyuncular.values()]
      .sort((a, b) =>
        kodlu ? a.kod.localeCompare(b.kod, 'tr') : a.isim.localeCompare(b.isim, 'tr')
      )
      .map((o) => oyun.olcme.rapor(o));
    console.log(`[veri] tüm karneler üretildi — ${raporlar.length} öğrenci · ${kodlu ? 'kodlu' : 'isimli'}`);
    yanit.send(karne.tumKarneler(raporlar, oyun.ayar, kodlu));
  });
  ...
  // ---- ℹ️ Etkinlik Bilgisi (kazanımlar · CHC · veli özeti) ----
  // Kaynak: proje kökündeki ETKINLIK_BILGI.json. Her istekte okunur ki dosya
  // güncellenince sunucuyu yeniden başlatmak gerekmesin. Öğrenciye GİTMEZ.
  rota.get('/etkinlik-bilgi', (istek, yanit) => {
    try {
      const dosya = path.join(__dirname, '..', 'ETKINLIK_BILGI.json');
      yanit.json(JSON.parse(fs.readFileSync(dosya, 'utf8')));
    } catch (e) {
      console.log(`[etkinlik] ETKINLIK_BILGI.json okunamadı: ${e.message}`);
      yanit.status(500).json({ hata: 'Etkinlik bilgisi okunamadı.' });
    }
  });

  // ---- "Listeyi İndir": güncel ogrenciler.json (depoya konup push'lanır) ----
  rota.get('/liste.json', (istek, yanit) => {
    console.log('[liste] ogrenciler.json indirildi');
    yanit.setHeader('Content-Type', 'application/json; charset=utf-8');
    yanit.setHeader('Content-Disposition', 'attachment; filename="ogrenciler.json"');
    yanit.send(liste.jsonMetni());
  });
