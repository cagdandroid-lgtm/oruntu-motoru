...
class Oyun extends EventEmitter {
  ...
  sahnedenCikar(kod) {
    return oturum.sahnedenCikar(this, kod);
  }
  ...
  baslat({ seviye, mod, modlar: secilenModlar, karisik }) {
    ...
    console.log(
      `[oyun] başladı — grup:${gecerliGrup} seviye:${gecerliSeviye} ` +
        `mod:${gecerliModlar.join('+')}${karisikMi ? ' (karışık)' : ' (sırayla)'} (${this.havuz.length} soru)`
    );
    this.sonrakiSoru();
  ...
  cevapVer(socketId, secim) {
    ...
    const sonucVeri = modlar.dogrula(this.soru, secim);
    ...
    if (typeof sonucVeri.puan === 'number') {
      ...
      kazanilan = sonucVeri.puan + (dogruMu ? HIZ_BONUSU[this.dogruSirasi] || 0 : 0);
    } else if (dogruMu) {
    ...
    this.cevaplar.set(oyuncu.anahtar, {
    ...
    this.olcme.ekle({
      soru: sonucVeri.kategori ? { ...this.soru, tur: sonucVeri.kategori } : this.soru,
      ayar: this.ayar,
      oyuncu,
