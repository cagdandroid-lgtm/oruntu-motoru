...
class Oyun extends EventEmitter {
  constructor() {
    ...
    this.on('bireyselHepsiBitti', () => this._bireyselKapanis());
  ...
  devam() {
    ...
    if (this.bireyselMi) {
      ...
      // Duraklatmada bekleyen geçişler şimdi yapılır (kimse takılı kalmaz)
      for (const o of this.oyuncular.values()) {
        if (o.ilerleme && o.ilerleme.bekliyor) {
          o.ilerleme.bekliyor = false;
          bireysel.sonraki(this, o);
        }
      }
      this.emit('degisti');
      return;
  ...
  // Geç katılan ya da yeniden bağlanan öğrenci kaldığı sorudan devam eder
  bireyselDevamEt(oyuncu) {
    if (!this.bireyselMi || this.durum !== 'oynaniyor') return;
    bireysel.yenidenBagland(this, oyuncu);
  }

  // Bireysel ilerlemede öğrencinin o anki sorusu
  oyuncununSorusu(oyuncu) {
    if (!this.bireyselMi || !oyuncu || !oyuncu.ilerleme) return null;
    return this.havuz[oyuncu.ilerleme.indeks] || null;
  }
  ...
  // Herkes kendi listesini bitirdi → SONUÇ
  _bireyselKapanis() {
    if (this.durum === 'bitti') return;
    this.durum = 'bitti';
    console.log('[oyun] bireysel etkinlik bitti — herkes kendi listesini tamamladı');
