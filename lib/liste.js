...
// Geçerli gruplar data/gruplar.json'dan gelir (p, e, u). Eski "i"/"c"
// değerleri gruplar.normalize() ile "u"ya çevrilir.
const { GRUPLAR } = gruplar;
const GRUP_ADI = Object.fromEntries(gruplar.TANIMLAR.map((g) => [g.kod, g.ad]));
...
function yukle() {
  try {
    ...
    gruplar.eskiKodlariKaydet(ogrenciler); // "eski_kod" alanları da eşlemeye katılır
...
function temizle(kayit) {
  // Eski dosyadan I-04 / "i" gelirse U-04 / "u" olarak okunur
  const kod = gruplar.kodNormalize(kayit && kayit.kod);
  const isim = String((kayit && kayit.isim) || '').trim();
  const grup = gruplar.normalize(kayit && kayit.grup);
  if (!kod || !isim || !GRUPLAR.includes(grup)) return null;
  const temiz = { kod, isim, grup, aktif: kayit.aktif !== false };
  // Birleşme kaydı korunur (yoksa "Listeyi İndir" bu alanı silerdi)
  if (kayit.eski_kod) temiz.eski_kod = String(kayit.eski_kod).trim().toUpperCase();
  return temiz;
}
...
function grupKartlari(grup) {
  const g = gruplar.normalize(grup);
  return tumu()
    .filter((o) => o.grup === g && o.aktif)
...
function ekle({ isim, grup }) {
  ...
  const temizGrup = gruplar.normalize(grup);
...
function guncelle(kod, degisiklik) {
  ...
  if (degisiklik.grup !== undefined) {
    const g = gruplar.normalize(degisiklik.grup);
...
function panelListesi({ grup = 'hepsi', durum = 'hepsi', arama = '' } = {}) {
  ...
  const g = grup === 'hepsi' ? 'hepsi' : gruplar.normalize(grup);
  return tumu()
    .filter((o) => (g === 'hepsi' ? true : o.grup === g))
...
function grupOzeti() {
  return gruplar.TANIMLAR.map(({ kod: g, ad, emoji, renk, sis }) => ({
    grup: g,
    ad,
    emoji,
    renk,
  ...
  return JSON.stringify(
    {
      ...
      ogrenciler: ogrenciler.map(({ kod, isim, grup, aktif, eski_kod }) =>
        eski_kod ? { kod, isim, grup, aktif, eski_kod } : { kod, isim, grup, aktif }
      ),
    },
    null,
    2
