...
//
// İki çıktı kipi vardır:
//   isimli (varsayılan) → veli için; öğrencinin adı yazılır
//   kodlu               → araştırma/arşiv için; ad yerine KOD yazılır,
//                         belgenin hiçbir yerinde isim geçmez.
...
// Karnede görünecek ders etiketi: bu öğrencinin kayıtlarındaki etiket,
// yoksa oturumun güncel etiketi.
function dersEtiketi(rapor, ayar) {
  if (rapor.dersEtiketleri && rapor.dersEtiketleri.length) return rapor.dersEtiketleri.join(' · ');
  return (ayar && ayar.dersEtiketi) || '';
}

// Tek öğrencinin karne gövdesi (bir A4 sayfa).
// kodlu=true ise öğrencinin adı belgenin HİÇBİR yerinde geçmez.
function karneSayfasi(rapor, ayar, kodlu = false) {
  const ders = dersEtiketi(rapor, ayar);
  return `<section class="sayfa">
  <header class="ust">
    <div>
      <h1>🧩 Örüntü Motoru — Öğrenci Karnesi</h1>
      <p class="alt">UYCEP Logic · sınıf içi örüntü çalışması${ders ? ' · ' + kacir(ders) : ''}</p>
    </div>
  ...
  <div class="kimlik">
    ...
    <div><span class="etiket">Çalışma grubu</span><b>${kacir(gruplar.ad(ayar.grup))}</b></div>
    ${ders ? `<div><span class="etiket">Ders</span><b>${kacir(ders)}</b></div>` : ''}
  </div>
...
function tekKarne(rapor, ayar, kodlu = false) {
  return belge(
    `Karne — ${kodlu ? rapor.kod : rapor.isim}`,
    karneSayfasi(rapor, ayar, kodlu),
    kodlu
      ? 'Kodlu kip: belgede isim geçmez, yalnız öğrenci kodu yazar.'
      : 'Yazdırma penceresinde “Hedef: PDF olarak kaydet” seçilebilir.'
  );
}

function tumKarneler(raporlar, ayar, kodlu = false) {
  const govde = raporlar.map((r) => karneSayfasi(r, ayar, kodlu)).join('\n');
  return belge(
    kodlu ? 'Sınıf karneleri (kodlu)' : 'Sınıf karneleri',
    govde || '<p class="bos">Henüz öğrenci yok.</p>',
    `${raporlar.length} öğrenci · her öğrenci ayrı sayfada.` +
      (kodlu ? ' Kodlu kip: isim geçmez.' : '')
  );
}
