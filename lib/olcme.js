...
// Standart sütunların SONUNA eklenen ek sütunlar (standart 13 sütun bozulmaz):
//  - misafir       : her dışa aktarımda — araştırma setini süzmek için
//  - ders_etiketi  : öğretmenin oturum açarken girdiği serbest etiket
//                    ("2. Ders · 12 Eylül") — oturumları ayırt etmek için
//  - ogrenci_ad    : yalnız isimli (veli raporu) dışa aktarımda
const MISAFIR_SUTUNU = 'misafir';
const DERS_SUTUNU = 'ders_etiketi';
...
const KATEGORI_ETIKET = {
  ...
  tersine: 'Tersine örüntü (geriye çalışma)',
  uzak: 'Uzak terim bulma',
  'kendi-oruntu': 'Kendi kurduğu örüntü',
  'kendi-tutarli': 'Kendi örüntüsü — tek kurallı ✅',
  'kendi-tutarsiz': 'Kendi örüntüsü — kural bozuluyor',
};

// CHC: kayda her zaman DAR yetenek kodu yazılır (lib/chc.js; moda göre
// Gf-I / Gf-RQ / Glr). Geniş kodlar ("Gf", "Gq") artık üretilmez.
const chc = require('./chc');
const chcAlanlari = (soru) => chc.kayitEtiketi(soru);
...
class OlcmeDeposu {
  ...
  ekle({ soru, ayar, oyuncu, sonuc, sureSn, deneme = 1, ipucu = false, turNo }) {
    const kayit = {
      ...
      oyun: OYUN_ADI,
      // Çoklu mod turlarında her soru KENDİ modunu yazar
      set_veya_paket: `${ayar.grup}-${ayar.seviye}-${soru.mod || ayar.mod}`,
      grup: ayar.grup,
      ...
      // ---- standart şemanın sonuna eklenen sütunlar ----
      misafir: oyuncu.misafir ? 'evet' : 'hayir',
      ders_etiketi: ayar.dersEtiketi || '',
      // ---- dışa aktarılmayan iç alanlar ----
      mod: soru.mod || ayar.mod, // mod bazlı öğrenci raporu için
  ...
  csv({ isimli = false, isimCozucu = null } = {}) {
    const basliklar = isimli
      ? [...SUTUNLAR, MISAFIR_SUTUNU, DERS_SUTUNU, ISIM_SUTUNU]
      : [...SUTUNLAR, MISAFIR_SUTUNU, DERS_SUTUNU];
    ...
    for (const k of this.gecerliKayitlar()) {
      ...
      hucreler.push(alanKacir(k.ders_etiketi || ''));
      if (isimli) {
  ...
  oncekiOturumuYukle(metin, dosyaAdi = 'önceki oturum') {
    ...
    for (const satir of tablo.slice(1)) {
      ...
      // Birleşme öncesi CSV'lerde I-04 / C-02 gibi kodlar U-04 / U-22'ye eşlenir
      const kod = gruplar.kodNormalize(satir[dizin.ogrenci_kod]);
  ...
  oncekiKayit(kod, isim) {
    ...
    return (
      ...
      ogrenciler.get('kod:' + gruplar.kodNormalize(kod)) ||
      null
    );
  }
  ...
  rapor(oyuncu) {
    ...
    for (const k of kayitlar) {
      ...
      // Mod bazlı doğruluk ve süre (CHC bilgisi raporda YER ALMAZ)
      const md = modHarita.get(k.mod) || { mod: k.mod, toplam: 0, dogru: 0, sureToplam: 0, sureSayisi: 0 };
      md.toplam++;
      if (k.sonuc === 'dogru') md.dogru++;
      if (k.sure_sn !== '' && Number.isFinite(Number(k.sure_sn))) {
        md.sureToplam += Number(k.sure_sn);
        md.sureSayisi++;
      }
      modHarita.set(k.mod, md);
    }
    ...
    return {
      ...
      // Bu öğrencinin kayıtlarında geçen ders etiketleri (karne başlığı için)
      dersEtiketleri: [...new Set(kayitlar.map((k) => k.ders_etiketi).filter(Boolean))],
      anahtar: oyuncu.anahtar,
      skor: oyuncu.skor,
      toplam,
      dogru,
      ...
      modlar: [...modHarita.values()]
        .map(({ mod, toplam: t, dogru: d, sureToplam, sureSayisi }) => ({
          mod,
          toplam: t,
          dogru: d,
          yuzde: yuzdele(d, t),
          ortalamaSureSn: sureSayisi ? iki(sureToplam / sureSayisi) : null,
        }))
        .sort((a, b) => b.toplam - a.toplam),
      degisim,
    };
