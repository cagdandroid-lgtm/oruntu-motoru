...
// İsimli (veli raporu) dışa aktarımda standart sütunların SONUNA eklenir.
...
// Öğrenci kodu: E-07, I-03 … (grup harfi + sıfır dolgulu sıra)
function kodUret(grup, sira) {
  const harf = String(grup || 'e').slice(0, 1).toUpperCase();
  return `${harf}-${String(sira).padStart(2, '0')}`;
}
...
class OlcmeDeposu {
  ...
  csv({ isimli = false, isimCozucu = null } = {}) {
    const basliklar = isimli ? [...SUTUNLAR, ISIM_SUTUNU] : [...SUTUNLAR];
